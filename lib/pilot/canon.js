 ...
 */
thing.metadata = {
    name: 'thing', // optional if there is another way to work it out ...
    description: 'thing is an example command',
    params: [{
        name: 'param1',
        description: 'an example parameter',
...
function thing(param1) {
    console.log(param1);
    var env = canon.getEnvironment();
}
...
function Command(commandSpec) {
    ...
    this.params.forEach(function(param) {
        if (!param.name) {
            throw new Error('In ' + this.name + ': all params must have a name');
        }
        upgradeType(this.name, param);
    }, this);
};
Command.prototype = {
