...
SelectionType.prototype.parse = function(str) {
    ...
    if (hasMatched) {
        ...
        if (completions.length > 0) {
            return new Conversion(null, Status.INCOMPLETE, '', completions);
        }
        else {
            var msg = 'Can\'t use \'' + str + '\'.';
            return new Conversion(null, Status.ERROR, msg, completions);
        }
    }
};
...
function ArrayType(typeSpec) {
    if (!typeSpec.subtype) {
        console.error('Array stypeSpec is missing subtype. Assuming text.' +
